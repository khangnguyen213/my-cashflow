            ...
            <Table>
              ...
              <TableBody>
                  ...
                  if (item.getLoanInterest() > 0) {
                    title =
                      item.getType() === AssetType.JOB
                        ...
                        : 'Loan interest: ' + item.getTitle();
                    amount = numberToDollar(-item.getLoanInterest());
                  } else {
                    title = item.getTitle() + ' expense';
                    amount = numberToDollar(item.getCashflow());
                  }
                  return (
                ...
                {childcareExpenses.length > 0 && (
                  <TableRow>
                    <TableCell className="font-medium">Childcare</TableCell>
                    <TableCell className="text-right">
                      {numberToDollar(
                        childcareExpenses[0].getExpensePerChild()
                      )}{' '}
                      x {childcareExpenses[0].getQty()} ={' '}
                      {numberToDollar(childcareExpenses[0].getCashflow())}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
